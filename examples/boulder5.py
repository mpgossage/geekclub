...
def load_level(event):
    """Load level.txt"""
    f = open('boulder-levels/levels.txt')
    lines = f.readlines()
    level = lines[0]
    f.close()
    display = "Load from what file? (1-" + (level) + ") "
    load = askinteger((display), (display))
    if int(load) > int(level) or int(load) < 1:
        print("Out of range")
    else:
        file = open("boulder-levels/save-" + str(load) + ".txt")
        print(file)
        ...
        world.status = 'play'
        banner('Level loaded!')
        future_action(clear_banner,1000)
        mainloop()
...
def save_level(event):
    if world.status != 'end':
        ...
        level = lines[0]
        f.close()
        f = open('boulder-levels/levels.txt', 'w')
        level = int(level) + 1
        f.write(str(level))
        f.close()
        level = str(level)
        f = open('boulder-levels/save-' + (level) + '.txt', 'w')
        for row in world.landscape:
            srow = [r.what if r else 'None' for r in row]
            f.write(','.join(srow) + '\n')
        banner('Level saved!')
